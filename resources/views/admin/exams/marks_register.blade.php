                    ...
                    @if(!empty($getStudentClass) && count($getStudentClass) > 0)
                        ...
                        <tr>
                            ...
                            <td colspan="{{ count($getSubject) + 3 }}" class="text-center">
                                    ...
                                    <a target="_blank" href="{{ url('admin/exam-result/print?exam_id='.Request::get('exam_id').'&student_id='.$value->id ) }}" class="btn btn-secondary">Print</a>
...
<script>
    $('.submitForm').submit(function(e){
        ...
        $.ajax({
            type: 'POST',
            url: '{{ url('admin/exams/submit-marks-register') }}',
            data: $(this).serialize(),
            dataType: "json",
    ...
    $('.saveSingle').click(function(e){
       ...
       var exam_marks = $('#exam_marks_'+subject_id+student_id).val();
        ...
        $.ajax({
            type: 'POST',
            url: '{{ url('admin/exams/single-submit-marks-register') }}',
            data: {
                _token: '{{ csrf_token() }}',
                id: id,
